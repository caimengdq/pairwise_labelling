...
class ASAP():
    ...
    def compute_prob_cmps(self):
        ...
        if self.selective_eig:
            ...
            prob_cmp = np.divide(prob_cmp.T, np.amax(prob_cmp, 1), out=np.zeros_like(prob_cmp.T),
                                 where=np.amax(prob_cmp, 1) != 0).T
        else:
    ...
    def kl_divergence_approx(self, mean_1, var_1, mean_2, var_2):
        ...
        '''
        total = np.sum(np.log(var_2)) - np.sum(np.log(var_1)) + sum(var_1 / var_2) + np.dot(1 / var_2,
                                                                                            (mean_1 - mean_2) ** 2)  #
        return total
...
class TrueSkillSolver():
    ...
    def solve(self, G, num_iters=6, save=True):
        ...
        for ii in range(0, num_iters):
            # print(np.reshape(Ps[G],np.shape(G)).shape, np.shape(G), self.Pgs.shape)
